      ...
      do i=1,nx+1
        do j=1,ny
           ...
           S = (1.0/6.0)*u*(1.0-dt*dt*u*u/(dx*dx))*R2
           Gy(i-1,j+1) = Gy(i-1,j+1) - (dt/dx)*v*S
           if(v .lt. 0.0)then 
              jj = j
           else
              jj = j-1 
      ...
      do j=1,ny+1
        do i=1,nx
           ...
           R2 = co(i,jj+1) - 2.0*co(i,jj) + co(i,jj-1)
           S = (1.0/6.0)*v*(1.0-dt*dt*v*v/(dy*dy))*R2
           Fx(i+1,j-1) = Fx(i+1,j-1) - (dt/dy)*u*S
           if(u .lt. 0.0)then 
              ii = i
           else
              ii = i-1 
